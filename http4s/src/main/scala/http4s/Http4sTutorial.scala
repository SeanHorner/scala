...
object Http4sTutorial extends IOApp {
  ...
  // movie database server simulator
  type Actor = String
  case class Movie(id: String, title: String, year: Int, actors: List[Actor], director: String)
  case class Director(firstName: String, lastName: String) {
    override def toString = s"$firstName $lastName"
  }
  case class DirectorDetails(firstName: String, lastName: String, movieCount: Int, genre: String)

  // internal "database" stand-in
  val snjl: Movie = Movie(
    "6bcbca1e-efd3-411d-9f7c-14b872444fce",
    "Zack Snyder's Justice League",
    2021,
    List("Henry Cavill", "Gal Godot", "Ezra Miller", "Ben Affleck", "Jason Momoa"),
    "Zack Snyder"
  )
  ...
  // (wrapper around our response type that takes unique responses in an Option wrapper and formats them into a
  // flat, generic style, denoted as F[] ).
  // HttpRoutes[F]
    ...
    HttpRoutes.of[F] {
      ...
      case GET -> Root / "movies" / UUIDVar(movieId) / "actors" =>
        findMovieById(movieId).map(_.actors) match {
          ...
          case _ => NotFound(s"No movie with id '$movieId' found in database.")
        }
    }
  }
  ...
  val directorDetailsDB: mutable.Map[Director, DirectorDetails] =
    mutable.Map(Director("Zack", "Snyder") -> DirectorDetails("Zack", "Snyder", 12, "action"))
